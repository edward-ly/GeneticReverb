...
You can start using the plugin right away (without the need to compile from source code) by extracting the contents of the provided `.zip` files and then copying the `.dll` (Windows) or `.vst` (Mac) file into the VST plugins directory specified by your desired host application.
